    ...
    public ResponseEntity<GenericApiResponse<BookmarkQuestionResponseDto>> bookmarkQuestion(@PathVariable Long questionId) {
        try {
            ...
            ErrorResponse errorResponse = ErrorResponse.builder()
                    ...
                    .stackTrace(Arrays.toString(e.getStackTrace()))
                    .build();
            GenericApiResponse<BookmarkQuestionResponseDto> response = ApiResponseBuilder.buildErrorResponse(BookmarkQuestionResponseDto.class, e.getMessage(), 401, errorResponse);
