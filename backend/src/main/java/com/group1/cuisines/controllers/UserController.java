...
public class UserController {
    ...
    public ResponseEntity<?> getUserById(
        ...
        String currentUsername = authentication != null
            ...
            : null;
        try {
            UserProfileDto userProfile = userService.getUserProfileById(
                userId,
                currentUsername
            );
    ...
    public ResponseEntity<?> unfollowUser(@PathVariable Integer userId) {
        ...
        if (followerId == null || userId == null) {
            return ResponseEntity.ok(
                new ErrorResponse(204, "Invalid user data")
            );
        }
        if (userId.equals(followerId)) {
            return ResponseEntity.badRequest()
                .body(new ErrorResponse(400, "Cannot unfollow yourself"));
        }

        boolean result = userService.unfollowUser(userId, followerId);
        if (!result) {
            return ResponseEntity.ok(
                new ErrorResponse(209, "Follow relationship does not exist")
            );
        }

        UserProfileDto followedUserDto = userService.getUserProfileDtoById(
            userId
        );
    ...
    public ResponseEntity<?> followUser(@PathVariable Integer userId) {
        ...
        if (userId.equals(followerId)) {
            return ResponseEntity.badRequest()
                .body(new ErrorResponse(400, "Cannot follow yourself"));
        }

        boolean result = userService.followUser(userId, followerId);
        if (!result) {
            return ResponseEntity.ok(
                new ErrorResponse(209, "Already following")
            );
        }
        UserProfileDto followedUserDto = userService.getUserProfileDtoById(
            userId
        ...
        return ResponseEntity.ok(
            new SuccessResponse<>(200, followedUserDto, "Followed successfully")
        );
    }
    ...
    public ResponseEntity<?> getUserFollowing(@PathVariable Integer userId) {
        ...
        if (userId == null) {
            return ResponseEntity.ok(
                new ErrorResponse(204, "Invalid user ID provided")
            );
        }
        ...
        if (following.isEmpty()) {
            return ResponseEntity.ok(
                new ErrorResponse(204, "User is not following anyone")
            );
        } else {
            Integer finalCurrentUserId = currentUserId;
            Set<UserDto> followingDto = following
                .stream()
                .map(
                    user ->
                        UserDto.builder()
    ...
    public ResponseEntity<?> getUserFollowers(@PathVariable Integer userId) {
        ...
        if (followers.isEmpty()) {
            return ResponseEntity.ok(
                new ErrorResponse(204, "User is not followed by anyone")
            );
        } else {
            Integer finalCurrentUserId = currentUserId;
            Set<UserDto> followingDto = followers
                .stream()
                .map(
                    user ->
                        UserDto.builder()
